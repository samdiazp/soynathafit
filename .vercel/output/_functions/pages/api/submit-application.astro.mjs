...
const POST = async ({ request }) => {
  try {
    ...
    const { data: existing } = await supabase.from("user_applications").select("id, status").eq("email", data.email.toLowerCase()).in("status", ["PENDING", "APPROVED"]).maybeSingle();
    if (existing) {
      return new Response(
        JSON.stringify({
          error: true,
          message: "Ya existe una solicitud con este correo. Por favor contacta soporte si necesitas ayuda."
        }),
